...
% 初始化结果表格
results = cell(length(cases) * length(n_range) * length(strategies), 13);
row_index = 1;
...
for i = 1:length(cases) % 遍历所有情况
    ...
    for n = n_range % 遍历数量范围
        ...
        for j = 1:length(strategies) % 遍历所有策略
            % 计算当前情况和策略下的总成本和次品率
            [total_cost, defective_rate, cost_1, cost_2, loss_1, loss_2, cost_final, loss_final, cost_3] = calc_total_cost(cases(i), strategies(j).detect_parts1, strategies(j).detect_parts2, ...
                                                           strategies(j).detect_final, strategies(j).dismantle, n);
            % 存储结果到表格
            results{row_index, 1} = i; % 情况编号
            results{row_index, 2} = n; % 数量
            results{row_index, 3} = total_cost; % 总成本
            results{row_index, 4} = defective_rate; % 次品率
            results{row_index, 5} = cost_1; % 零配件1的检测成本
            results{row_index, 6} = cost_2; % 零配件2的检测成本
            results{row_index, 7} = loss_1; % 零配件1的损失成本
            results{row_index, 8} = loss_2; % 零配件2的损失成本
            results{row_index, 9} = cost_final; % 成品的检测成本
            results{row_index, 10} = loss_final; % 成品的损失成本
            results{row_index, 11} = cost_3; % 拆解不合格成品的成本
            results{row_index, 12} = defective_rate; % 次品率
            results{row_index, 13} = strategy_explanations{j}; % 策略解释
            row_index = row_index + 1;
            fprintf('%s: 总成本 = %.2f, 零配件1的检测成本 = %.2f, 零配件2的检测成本 = %.2f, 零配件1的损失成本 = %.2f, 零配件2的损失成本 = %.2f, 成品的检测成本 = %.2f, 成品的损失成本 = %.2f, 拆解不合格成品的成本: %.2f, 次品率 = %.2f\n', ...
                strategy_explanations{j}, total_cost, cost_1, cost_2, loss_1, loss_2, cost_final, loss_final, cost_3, defective_rate); 
        end
    end
end

%% 将结果转换为表格
results_table = cell2table(results, 'VariableNames', {'Case', 'Quantity', 'TotalCost', 'DefectiveRate1', ...
    'Cost1', 'Cost2', 'Loss1', 'Loss2', 'CostFinal', 'LossFinal', 'Cost3', 'DefectiveRate2', 'StrategyExplanation'});

% 将结果保存到Excel文件
filename = 't2_results.xlsx';
writetable(results_table, filename);

%% 计算总成本的函数
function [total_cost, defective_rate, cost_1, cost_2, loss_1, loss_2, cost_final, loss_final, cost_3] = calc_total_cost(current_case, detect_parts1, detect_parts2, detect_final, dismantle, n)
    n_1 = n; % 零配件1的数量
    n_2 = n; % 零配件2的数量
